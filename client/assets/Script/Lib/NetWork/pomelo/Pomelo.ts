...
export class Pomelo extends EventEmitter {
    ...
    sendMessage(reqId: number, route: string, msg: any) {
        ...
        //   console.log("crypto==========");
        //   msg = JSON.stringify(msg);
        //   let sig = protocolSafe.signString(msg);
        //   msg = sig;
        // }
    ...
    heartbeat(data: any) {
        ...
        let obj = Package.encode(Package.TYPE_HEARTBEAT, null);
        if (this.heartbeatTimeoutId) {
            clearTimeout(this.heartbeatTimeoutId);
            this.heartbeatTimeoutId = undefined;
        }
    ...
    handshake(data: any) {
        ...
        // 给服务器发送握手确认协议
        let obj = Package.encode(Package.TYPE_HANDSHAKE_ACK, null);
        this.send(obj);
        // 成功接受到服务器握手协议
    ...
    deCompose(msg: any) {
        ...
        if (Protobuf && this.serverProtos[route]) {
            ...
            let body = Protocol.strdecode(msg.body);
